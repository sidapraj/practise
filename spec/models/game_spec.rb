...
RSpec.describe Game, type: :model do
  ...
  describe 'correctness' do
    ...
    let(:game) do
      build_stubbed(:game, good_answers_count: good_answers_count, 
      bad_answers_count: bad_answers_count)
    end

    context 'when good answer count is 0' do
      let(:good_answers_count) { 0 }
      context 'when bad answer count is 0' do 
        let(:bad_answers_count) { 0 }
           it { is_expected.to eq('-') }
      end

      context 'when bad answer count is 1' do 
        let(:bad_answers_count) { 1 }
           it { is_expected.to eq('0.0%') }
      end
    end

    context 'when good answer count is 1' do
    ...
    context 'when good answer count is 2' do
      let(:good_answers_count) { 2 }
      context 'when bad answer count is 4' do 
